    ...
    const setData = (data: T | null): void => {
        ...
        if (data !== null) {
            try {
                ...
                document.cookie = `${tokenName}=${encoded};path=/;max-age=${maxAgeSec};SameSite=Strict;${secureFlag}`;
            } catch (e) {
                console.error(`Error encoding data for ${tokenName}: `, e);
            }
        } else {
            document.cookie = `${tokenName}=null;path=/;expires=Thu, 01 Jan 1970 00:00:01 GMT;SameSite=Strict;${secureFlag}`
        }
